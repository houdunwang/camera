...
const onSubmit = async () => {
  if (!form.value.secret.trim()) ElMessage({ message: '口令不能为空', type: 'warning' })
  else
    try {
      const res = await getSecret(form.value)
      ElMessage.success('口令正确')
...
<template>
  ...
  >
    <h3 class="text-center text-gray-50 opacity-90 text-base font-sans font-light">请输入口令</h3>
    <!-- <div class="text-gray-200 text-xs opacity-50 mb-3">网站会员中心查看口令</div> -->
    <el-input
      v-model="form.secret"
